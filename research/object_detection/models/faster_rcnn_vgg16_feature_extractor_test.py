...
"""Tests for models.faster_rcnn_vgg16_feature_extractor.

TESTS WERE COPIED FROM INCEPTION RESNET AND HAVE NOT BEEN FIXED YET
...
class FasterRCNNVGG16FeatureExtractor(tf.test.TestCase):

  def _build_feature_extractor(self, first_stage_features_stride):
    return frcnn_vgg16.FasterRCNNVGG16FeatureExtractor(
        is_training=False,
  ...
  def test_extract_box_classifier_features_returns_expected_size(self):
    ...
    with self.test_session() as sess:
      sess.run(init_op)
      features_shape_out = sess.run(features_shape)
      self.assertAllEqual(features_shape_out, [2, 8, 8, 4096])
